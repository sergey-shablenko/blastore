...
export type BuildStandard<TValidate extends Record<string, StandardSchemaV1>, TKeyMode extends {
    ...
    set<TSetKey extends IndexableKeyOf<TValidate>>(key: TSetKey, value: NonNullable<TValidate[TSetKey]['~standard']['types']>['output'], options?: KeyVariables<TSetKey> & {
        validate?: boolean;
        out?: {
            error?: Error;
        };
    ...
    emit<TEmitKey extends IndexableKeyOf<TValidate>>(key: TEmitKey, action: 'set' | string, data: NonNullable<TValidate[TEmitKey]['~standard']['types']>['output'], options?: KeyVariables<TEmitKey> & {
        validate?: boolean;
        out?: {
            error?: Error;
        };
    ...
    untypedEmit<TFDeserialize extends boolean>(key: string, action: 'set' | string, data: Switch<TFDeserialize, TOutput, NonNullable<TValidate[keyof TValidate]['~standard']['types']>>, options?: {
        validate?: boolean;
    ...
    buildKeyApi: <TApiKey extends IndexableKeyOf<TValidate>>(key: TApiKey, options?: KeyVariables<TApiKey> & {
        ...
        set(value: NonNullable<TValidate[TApiKey]['~standard']['types']>['output']): MaybePromisify<TKeyMode[TApiKey], boolean>;
        ...
        emit(action: 'set' | string, data: NonNullable<TValidate[TApiKey]['~standard']['types']>['output']): MaybePromisify<TKeyMode[TApiKey], boolean>;
    };
}>;
