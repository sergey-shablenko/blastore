...
> {
  ...
  keyMode: TKeyMode;
  defaultSerialize?: (
    v: NonNullable<TValidate[keyof TValidate]['~standard']['types']>['output']
  ) => TInput;
  defaultDeserialize?: (
    v: TOutput
  ) => NonNullable<TValidate[keyof TValidate]['~standard']['types']>['output'];
  serialize?: Partial<{
    [K in keyof TValidate]: (
      v: NonNullable<TValidate[K]['~standard']['types']>['output']
    ) => TKeyMode[K] extends 'async' ? Promise<TInput> : TInput;
  }>;
  deserialize?: Partial<{
    [K in keyof TValidate]: (
      v: Awaited<TOutput>
    ) => TKeyMode[K] extends 'async'
      ? Promise<NonNullable<TValidate[K]['~standard']['types']>['output']>
      : NonNullable<TValidate[K]['~standard']['types']>['output'];
  }>;
...
> {
  ...
  validate: TValidate;
  defaultSerialize?: (
    v: Exclude<ReturnType<TValidate[keyof TValidate]>, Error>
  ) => TInput;
  defaultDeserialize?: (
    v: TOutput
  ...
  serialize?: Partial<{
    [K in keyof TValidate]: (
      v: Exclude<ReturnType<TValidate[K]>, Error>
    ) => TInput;
  }>;
  deserialize?: Partial<{
    [K in keyof TValidate]: (
      v: TOutput
    ) => Exclude<ReturnType<TValidate[K]>, Error>;
  }>;
  validateOnGet?: boolean;
  validateOnSet?: boolean;
  validateOnEmit?: boolean;
...
> {
  ...
  defaultDeserialize?: (
    v: TOutput
  ) => Promise<Exclude<ReturnType<TValidate[keyof TValidate]>, Error>>;
  serialize?: Partial<{
    [K in keyof TValidate]: (
      v: Exclude<Awaited<ReturnType<TValidate[K]>>, Error>
    ) => Promise<TInput>;
  }>;
  deserialize?: Partial<{
    [K in keyof TValidate]: (
      v: TOutput
    ) => Promise<Exclude<Awaited<ReturnType<TValidate[K]>>, Error>>;
  }>;
  validateOnGet?: boolean;
  validateOnSet?: boolean;
  validateOnEmit?: boolean;
...
export type MaybePromisify<
  TInput extends 'async' | 'sync',
  TOutput,
> = TInput extends 'async'
  ? Promise<TOutput>
  : TInput extends 'sync'
    ? TOutput
    : never;
...
export type Switch<TFlag extends boolean, TType1, TTyp2> = TFlag extends true
  ? TType1
  : TTyp2;
